...
**`generated: true` and `outsideRoot: true` are the other two reasons a hit has no location**, and the
only ones the response can name for you. The rows come from the semantic tier, which sees the whole
compilation; the locations come from the syntax index, which scans the repo tree and prunes
`bin`/`obj`/`dist` first. A declaration the two disagree about arrives with an empty `file`, `line` and
`endLine`, which without a flag reads as an indexing failure. The two cases are not the same:

- **`generated`** — source-generator or build output under a pruned directory. The same field, spelled
  the same way, that `get_symbol` sets on the same symbol. There is no span to patch: the file is
  rewritten on every build.
- **`outsideRoot`** — a `Compile` item contributed from outside the repo root, which the syntax index
  never had in scope. Real source, and `get_symbol` will still hand you its path — a `../../…` one,
  pointing into a package — but not yours to edit. The test SDK's synthesized entry point
  (`AutoGeneratedProgram`, from `Microsoft.NET.Test.Sdk.Program.cs`) is the one nearly every test
  project has.

Both are emitted only when they apply, and never together.
...
syntax outline `line`/`endLine` come from — and `get_symbol`'s `members` component lists private members
too, so the count predicts the fetch. Through contract 3.53 it did not: the listing was filtered to
public/protected/internal surface, so `M9` on a helper-heavy type led to two rows with nothing naming
the seven it dropped. Two benign divergences remain, both stated in `get_symbol.md`: `M` is counted per
**declaration**, so each part of a partial type carries its own share while the listing merges every
part, and a nested type is counted by `N` yet still listed as a member.
