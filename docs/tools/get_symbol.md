...
declaration is source-generator output. Roslyn counts generated files as source, so `origin` says
...
Note this is a *ranked index lookup on the segment*, not fuzzy matching: it recovers a right name under
a wrong qualification, not a misspelled one. `candidates` is capped at ten. `totalCandidates` always states how many matched, and `truncated: true`
