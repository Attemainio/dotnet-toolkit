...
| `members` | For a type only: `[{symbolId, displayString, kind, line, shape, contentVersion}]` per member, plus `file` on a member declared in a different file from the type's own primary site (a partial); `null` otherwise. `line` is the member's signature line, matching `search_index`'s convention; `shape` is the same column `search_index` puts on a hit, and the response states its legend once as `shape` beside the list. Each row's `contentVersion` is **narrowed to `decl`**, matching what a row serves — a name, a location and a shape, never a body. It leases a signature/doc edit; a body edit built on it is rejected with `unleased_body`, which is correct rather than awkward: nothing here showed you the body |
...
`generated: true` marks the third case, and appears only when it applies: a symbol whose *every*
declaration is source-generator output. (`search_index` splits the same question one step finer on its
own rows, which carry no path at all to fall back on: `generated` there means the same thing, and
`outsideRoot` covers a declaration outside the repo root — a case this response answers by simply
showing you the `../../…` path.) Roslyn counts generated files as source, so `origin` says
`"source"` — while `declarationSites` deliberately excludes `obj/**`, leaving the array empty. In
source, and nowhere to be found, reads as a lookup bug without this field. There is no span to patch:
the file is rewritten on every build, so an edit there would be overwritten out from under you.
...
- `include: "members"` — a type's API surface with no bodies and none of the standard extras. Each row
  carries its own `line` and `shape`, so this is the call a search hit's `M…` points at: read the list,
  pick the member, then `source:code@from-to` on it — never a fetch of the whole type.
...
`-lineNumbers` drops the per-line `NN:` gutter. **The saving is real but small — about 6% on a typical
member, not the ~18% claimed before it was measured against the gutter render rather than against the
raw file.** What the gutter actually costs is its own width: under `toon` a rendered line carries the
block indent, the `NN: ` gutter, and the file's own leading whitespace; `-lineNumbers` removes only the
middle one and keeps a per-run `@start-end` header. It is a fixed cost per line, so the saving is
larger on short, shallow lines and smaller on long ones — expect single digits, not a fifth of the
payload. In exchange the component changes shape: instead of `[{line, text}]` it returns `[{lines, text}]`,
...
rows still come back so the response is never a dead end.

**Length is not the only way an outline fails to earn its keep.** `bodyOutlineNote` also fires on
**density**: a declaration past the 40-line bound whose outline averages worse than one landmark per 25
lines is mostly linear, so the rows describe almost none of it and the note says so —

```
"bodyOutlineNote":"declaration is 40 lines but the outline has only 1 entry - a mostly linear body,
 so this outline describes little of it and source:code (or a source:code@from-to range) is likely
 the better answer"
```

Before that check existed, the note fired only on short declarations, so a 40–150-line member of
sequential statements with one branch returned a near-empty outline and said nothing about it — and the
caller paid for the outline *and* the `source` fetch it needed anyway.

It is not a substitute for reading the
...
| `mechanicalFacts` | Server-computed structural facts as opaque JSON; `null` if the body changed since computed. Members carrying nothing (an empty `throws`/`awaits`/`writes`/`locks`/`implementsMembers`, a null `overrides`) are omitted rather than emitted empty — absence means "none" |
| `bodyOutline` | Control-flow landmarks inside a method-like body as `[{text, startLine, endLine, depth}]` in `format:"json"`/`"compact"` — under the default `toon` format, `depth` is dropped and nesting instead reads from two-space-per-level indentation on a raw `text,startLine,endLine` block (see below), the same raw-block treatment `source` gets and for the same reason. Purely syntactic (no semantic model, so it costs the same tier as `source`, not `mechanicalFacts`' semantic-model tier) — for navigating a long body without reading it. Covers `switch`/`case`, `if`, `foreach`/`for`/`while`/`do`, `catch`, `using`, `lock`; a bare `try`/`else`/`finally` carries no name or condition of its own and is omitted — its span is inferable from the parent row. `text` is a short label (e.g. `"switch(node)"`, `"if (name.Length > 3)"`), truncated to a 28-character budget with a trailing `..`, not a semantic summary. `depth` (or indentation, under `toon`) counts nesting among *other landmark rows only*, not raw syntax depth — a landmark buried inside several plain blocks isn't deeper than one actually nested inside another landmark. Absent (with an explanatory **`bodyOutlineNote`**, e.g. `"bodyOutline is not applicable to a Type symbol - only a method has an executable body to outline"`) for anything without an executable body of its own (a type, a field, an auto-property) — rather than both fields silently disappearing. `bodyOutlineNote` also appears — without suppressing the rows — whenever the outline is unlikely to be worth its cost, and is advisory only. Two triggers: the declaration (doc-comment-inclusive, matching `declarationSites`) is under 40 lines (`"source:code is likely cheaper than this outline"`), or it clears that bound but averages worse than one landmark per 25 lines, i.e. a mostly linear body whose outline describes little of it (`"declaration is N lines but the outline has only M entries - a mostly linear body …"`). Density, not length alone, is what decides whether an outline describes a body |
| `referenceCounts` | `{callers, tests}` for a member (never for a type), plus `{implementations, overrides}` — but each of those two only where the symbol's kind makes a non-zero answer possible: `implementations` for an interface, an unsealed class or an interface member; `overrides` for a virtual/abstract member. An enum, a static class or a plain method omits them rather than reporting a structural `0`, and the whole component is absent when nothing is left to report |
| `recentLog` | Recent dev-log entries touching this symbol, each flagged `current:true/false` against the live body |
| `members` | For a type only: `[{symbolId, displayString, kind, line, shape, contentVersion}]` per member. `line` is the member's own signature line — the same convention `search_index`'s `line` uses, so it is a navigation anchor rather than an edit anchor; a `validate_patch` span still comes from a `get_symbol` on that member, whose `declarationSites` starts at the doc comment. `file` is added only on a member declared in a different file from the type's own primary declaration site, so only a partial class pays for the column. `shape` is byte-identical to the column `search_index` would put on the same symbol (a test asserts it), and the legend is stated once as a top-level `shape` field beside the list rather than repeated per row — so a type's `M` count leads straight to a member list that says which member is worth opening and where it is, in one further call. **Private members are listed**, because `M` counts them: through contract 3.53 the listing was filtered to public/protected/internal surface and a `M9` type answered with two rows. Two divergences from `M` remain by construction — `M` is counted per *declaration*, so each part of a partial type carries its own share while this listing merges every part, and a nested type is counted by `N` yet still listed here. `contentVersion` is **narrowed to the `decl` layer**, because that is what a member row serves: a name, a location and a shape, never a body. It is enough to lease a signature or doc-comment edit; a body edit built on it returns `unleased_body`, the same as any other token that never held the body layer. Fetching the member itself with `source` is what hands over a body lease |
| `attributes` | This symbol's own (non-inherited) C# attributes as `[{name, arguments}]`; `name` strips a trailing `Attribute` suffix (e.g. `[Obsolete]` → `"Obsolete"`); `arguments` is a compact rendering of the constructor/named arguments **the author actually wrote**, truncated rather than reproduced in full for a long string. An argument the compiler supplies from the use site — a `[CallerFilePath]`/`[CallerLineNumber]`/`[CallerMemberName]`/`[CallerArgumentExpression]` parameter — is dropped, so an attribute written with empty brackets reports no arguments at all: xUnit v3's `[Fact]` and `[Theory]` declare exactly those parameters, and reporting them put an absolute machine path on the most common attribute in a test project. Absent when nothing was written. Absent when the symbol has no attributes. Suppressed (absent) when `source` is also requested |
...
`symbol_not_found` carries **`didYouMean`** — up to five ranked index hits for the last dotted segment
of the name that failed, each `{symbolId, name, kind}`. A miss is almost always a *near* miss (a wrong
namespace, a dropped containing type), so the answer is usually in the list and needs no `search_index`
round trip. It is absent when the segment is under three characters, when the name was a `sym_…` handle
(a stale id needs the id explanation, not a name guess), or when nothing ranked:

```json
{"error":"symbol_not_found","symbol":"Nowhere.Widget",
 "didYouMean":[{"symbolId":"sym_...","name":"Sample.Lib.Widget","kind":"Type"},
               {"symbolId":"sym_...","name":"Sample.Lib.IWidget","kind":"Type"}]}
```

It is a *ranked index lookup on the segment* first, which is what recovers a right name under a wrong
qualification. **When that returns nothing, an edit-distance scan answers a misspelling**: up to three
edits, case-insensitively, against every source symbol's own unqualified name, nearest first. Past that
a "correction" is a different word, so a name that is a typo of nothing still comes back bare rather
than with confident nonsense. The scan runs only after a symbol has already failed to resolve, so it
costs nothing on the normal path.

**Every tool that resolves a symbol name returns this**, not just `get_symbol`: `get_references`,
`get_call_hierarchy`, `get_call_slice`, `get_type_hierarchy` and `rename_symbol` carry the same
`didYouMean` on their own `symbol_not_found`.
Through contract 3.53 only `get_references` did, so the same miss through `get_symbol` came back with
nothing to act on. `candidates` is capped at ten. `totalCandidates` always states how many matched, and `truncated: true`
