...
**3. Skills vs. tool set.** For each of `skills/dotnet-change/SKILL.md` and
...
a missing doc row. `skills/dotnet-toolkit-init/SKILL.md` copies `.claude/rules/index.md` verbatim rather
than embedding its own tool table, so there is no second copy to check — confirm that is still true.
...
| `.claude/rules/index.md` | **the only always-loaded rule** — here *and* copied verbatim into consuming repos by init, so drift ships and is paid by every session and every subagent. It must carry only *when* and *where*: (a) **the router** — one row per tool mapping the wrong path to the right tool, covering every tool from Step 0. The manual's filename is **derived, not tabulated** (`<tool>.md`, with the four server/meta tools sharing `server.md`), stated once in the resolution section; a reintroduced third column is drift, since it duplicates a derivable fact and can name a file that no longer exists. (b) **the ToolSearch discipline rule** — load a tool by exact name via `select:`, never by describing the need. Measured: a natural-language query ranked unrelated tools above the right one and never surfaced `get_references`, so the router is load-bearing and this rule is what makes it reachable. (c) workspace readiness (`limitedBy`) and the response conventions, documented once here rather than per tool. (d) an exploring section naming `dotnet-explore` and when to skip it. (e) **the write mandate** — invoke `dotnet-change` before the first `.cs` change of a session, stated as a precondition in the `## Writing` section, not only as a bullet in the skills list; plus a write path matching `validate_patch`'s current arguments. (f) a standards table listing exactly the files present in `standards/`, whose "When" column doubles as the **reviewer's trigger table** — so each condition must be an *observable property of the code* (awaits/locks, hot path, public surface change), not a topic a reviewer can't match against retrieved source, and the always-loaded core it names must match `agents/dotnet-code-review.md`'s exactly. (g) the skill-invocation mandates, as triggers only — a skill's own description is already in context, so re-explaining what a skill *carries* is duplication this file pays for in every session and every subagent. **No `paths:` frontmatter**, and `${CLAUDE_PLUGIN_ROOT}` appears **only** in the sentence forbidding its use — a rule is delivered literally, never expanded, so a real path built from it would be dead text. Plugin files are instead reached by the documented join: `workspace_status`'s `pluginRoot:` + `docs/tools/<name>` or `standards/<name>`. That resolution section must be present, must match what `workspace_status` actually returns, and must name **exactly one** location per file — a per-repo override tier reappearing anywhere is the drift to flag. A write-procedure bullet or a standards *body* reappearing here is the drift to flag |
| `skills/dotnet-change/SKILL.md` | (a) its pre-edit standards step **defers to `index.md`'s standards table rather than re-listing the files** — the enumeration used to live here too and drifted against the table and the agent's list independently, so a reappearing per-file list is itself the finding; (b) it resolves standards as `<pluginRoot>/standards/<name>.md` via `workspace_status`, with no override tier and no `${CLAUDE_PLUGIN_ROOT}` written into a path; (c) **it owns the write-*decisions*** — when to dry-run, when to amend versus rebuild, the definition of done, raising `.editorconfig` severity with the user rather than unasked — which were moved out of the always-loaded layer and must not drift back. It does **not** own the write-time checklist: that moved to the `hint-write-checklist` hook (Step 5), and this file must keep pointing at it rather than carrying a second copy; (d) it does **not** restate `validate_patch`'s error codes, draft lifetimes or severity table: those belong to `docs/tools/validate_patch.md`, and a copy reappearing here is drift |
...
| `skills/dotnet-toolkit-init/SKILL.md` | that it still **copies `.claude/rules/index.md` verbatim** rather than embedding its own copy of the tool table or the standards-file list — an embedded copy reappearing here is the finding, since it would drift against the real rule and ship that drift to every consumer. Also the "what is deliberately *not* written" table and the uninstall list: every asset the plugin ships is in exactly one of write / not-written / uninstall, and the write and uninstall lists name the same files |
...
**5. Hooks and launch path.** All five hooks are `hook <name>` subcommands of the published server
...
`get_symbol` (not `grep`), plus `hooks/hooks.json` and `.mcp.json`:
   ...
   - **`hint-write-checklist` owns the write-time checklist**, and is the only hook matching an MCP
     tool rather than a built-in. Its text must still match the standards it summarizes, and it must
     stay once-per-session and fail-quiet — a version that fires on every patch, or that denies, is a
     finding. **It is the checklist's only owner**: the items must stay consistent with the standards
     that argue them (`security.md`, `testing.md`), and a copy reappearing in
     `skills/dotnet-change/SKILL.md` is drift — that skill points at the hook and must keep doing so.
     Matching an MCP tool name in `hooks.json` is verified working; don't "fix" it back to a built-in.
...
router *and* its own `docs/tools/<tool>.md`. Routing lives in the router and nowhere else; a skill earns
a line only when the change alters a *procedure* it owns. Update the skill body, not only its tool list,
so a caller reading the skill gets the same guidance a caller reading the code would.
