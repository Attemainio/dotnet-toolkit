---
name: dotnet-devlog
description: Use at the start of a coding task to check prior work and decisions, and at the end of any completed, partial, or abandoned change to record it. The development log stores WHAT/WHY/OBSERVATIONS/FIX entries with affected classes and domains, searchable via MCP tools.
---

# Development log discipline

This repo keeps a structured development log in weekly markdown files
(`devlog/<year>-W<week>.md`, project-root-level, not under `docs/`) managed by the
dotnet-toolkit MCP server.

**Hard rule: never Read, Grep, or edit `devlog/*.md` directly.** Always go
...
- `mcp__plugin_dotnet-toolkit_dotnet__devlog_search` with a free-text `query`
  ...
  `domain`, `tag`, `status`, `from`/`to` dates.
- Results are compact hit rows (id, date, title, status, classes, domain, score).
...
- `domain` — the module/folder/subsystem name.
