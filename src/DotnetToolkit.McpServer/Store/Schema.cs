...
internal static class Schema
{
    ...
    [
        ...
        new(17, "tool_call_events", ToolCallEvents),
    ];
    ...
    // What the server records about itself (retrieval_events) covers only its own tools, which cannot
    // measure the alternative it is being compared against: a Grep or a Read never reaches this process.
    // A PostToolUse hook sees EVERY tool call the harness dispatches, MCP or not, so this table is the
    // only place the two routes are measured on one instrument. Both directions are recorded because
    // they are priced differently: a tool RESPONSE is loaded into the model's context (input tokens, the
    // cheap side) while a tool REQUEST had to be generated by the model (output tokens, roughly 5x dearer
    // on Opus 5 and Haiku 4.5 alike), so one number for "tokens" would average away the expensive half.
    // token_estimator names what produced the counts, keeping a later recalibration distinguishable from
    // today's chars/4 approximation rather than silently mixed into it. tool_use_id is the harness's own
    // id for the call and is UNIQUE, so a replayed or duplicated hook delivery is ignored rather than
    // double-counted.
    private const string ToolCallEvents = """
        ...
        CREATE INDEX ix_tce_session ON tool_call_events(session_id, created_at);
        CREATE INDEX ix_tce_agent ON tool_call_events(agent_type, created_at);

        CREATE TRIGGER trg_tce_immutable BEFORE UPDATE ON tool_call_events BEGIN
            SELECT RAISE(ABORT, 'raw telemetry is immutable');
        END;
        """;
}
