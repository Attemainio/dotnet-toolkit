...
public static class PatchTools
{
    ...
    public static async Task<string> ValidatePatch(
        ...
        [Description("Whether to run Roslyn analyzers (CA/IDE rules) once the compile rungs are clean (default true). This is a WALL-CLOCK lever, not a token one -- measured, skipping analyzers cut a validation from ~545ms to ~78ms while the response itself cost about the same either way, since notAssessed grows to state what was skipped. Set false when only the compile/semantic result matters and latency is what you're trying to save -- checks.analyzers then reports ran:false with a skipReason instead of a verdict.")] bool runAnalyzers = true,
