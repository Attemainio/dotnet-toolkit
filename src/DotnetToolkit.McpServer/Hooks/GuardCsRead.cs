...
/// compiles, pointing at the <c>dotnet-read</c> skill instead.
/// </summary>
/// <remarks>
...
internal static class GuardCsRead
{
    ...
    public static HookOutcome Evaluate(HookPayload payload, HookContext context)
    {
        ...
        return HookOutcome.Deny($"""
            Blocked Read on {file}: it is compiled by {project}, so this repo's dotnet-toolkit MCP tools answer
            it more cheaply and completely than a raw file read - no truncation risk, and no irrelevant methods
            pulled in alongside the one you want.
            ...
            drift from the skill, and the skill is the one place it stays correct.
